...
VGG_WEIGHT_FILE = "/home/ubuntu/vgg16_weights.h5"
...
if __name__=="__main__":
    ...
    arg_parser.add_argument("--vgg-weights", type=str, default=VGG_WEIGHT_FILE)
    ...
    model.load_weights(args.vgg_weights)
    model.layers.pop()
    model.layers.pop()
    ...
    model.compile(optimizer=Adam(lr=args.lr, clipvalue=GRAD_CLIP),
                  loss="binary_crossentropy",
                  class_mode="binary")
    im = imread('data/100499/100499_301.jpg').astype("float32")
    im[:,:,0] -= 103.939
    im[:,:,1] -= 116.779
    im[:,:,2] -= 123.68
